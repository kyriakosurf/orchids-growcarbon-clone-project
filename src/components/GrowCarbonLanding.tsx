...
function ScrambledTexture() {
  ...
  React.useEffect(() => {
    const result: string[] = [];
    // Reduced from 180×20 to 60×10 for performance — still enough for visual effect
    for (let i = 0; i < 60; i++) {
      const words: string[] = [];
      for (let j = 0; j < 10; j++) {
...
function CtaRow({
  ...
  const isExpandable = !href && !!children;
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-4">
        ...
        <span className={`shrink-0 text-lg transition-transform duration-300 ${primary ? "text-black" : "text-[#d4a017]/50"} ${expanded ? "rotate-180" : ""}`} aria-hidden="true">
          {primary ? "↓" : "→"}
        </span>
      </div>
      {expanded && children && (
  ...
  return (
    <div className="min-h-screen bg-[#060604] text-white font-mono relative overflow-hidden">
      ...
      <main className="relative z-10 max-w-3xl mx-auto px-6 py-20 md:py-28">
        ...
        <div className="mb-8">
          <h1 className="text-3xl md:text-4xl font-bold tracking-tight">
            Grow<span className="text-[#d4a017]"><svg viewBox="0 0 2 3"
              ...
              fill="none" aria-hidden="true" focusable="false"><rect x="1" y="0" width="1" height="1" fill="currentColor" /><rect x="0" y="1" width="1" height="1" fill="currentColor" /><rect x="1" y="2" width="1" height="1" fill="currentColor" /></svg>arbon</span>
          </h1>
        ...
        {/* Intro Text */}
        <section className="space-y-6 text-sm md:text-base leading-[1.8] text-white/70 mb-14" aria-label="About Grow Carbon">
          <p>
        ...
        {/* SECTION 1 — CTA Rows */}
        <section className="space-y-3 mb-20" aria-label="Actions">
          <CtaRow
            primary
